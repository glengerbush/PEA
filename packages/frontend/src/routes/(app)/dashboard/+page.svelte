<script lang="ts">
	...
	const remoteIssueCount = $derived(
...
<div class="flex-1 space-y-4">
	...
	{#if !data.ingestionSources || data.ingestionSources?.length === 0}
		<div>
			<EmptyState
				buttonText={$t('app.dashboard_page.create_import')}
				header={$t('app.dashboard_page.no_import_header')}
				text={$t('app.dashboard_page.no_import_text')}
				click={() => {
					goto('/dashboard/imports');
				}}
			></EmptyState>
		</div>
	{:else}
		...
		<div class="space-y-4">
			{#if data.stats}
				<div class="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
					...
					<a href="/dashboard/imports" class="block">
						<Card.Root class="hover:border-primary/60 h-full transition-colors">
							<Card.Header
								class="flex flex-row items-center justify-between space-y-0 pb-2"
							>
					...
					<a href="/dashboard/remote-content-issues" class="block">
						<Card.Root class="hover:border-primary/60 h-full transition-colors">
							<Card.Header
								class="flex flex-row items-center justify-between space-y-0 pb-2"
							>
								<Card.Title class="text-sm font-medium"
									>Remote content issues</Card.Title
								>
								<ImageOff class="text-muted-foreground h-4 w-4" />
							</Card.Header>
							<Card.Content>
								<div
									class="text-2xl font-bold"
									class:text-destructive={remoteIssueCount > 0}
									class:text-green-600={remoteIssueCount <= 0}
								>
									{remoteIssueCount}
								</div>
								<p class="text-muted-foreground mt-1 text-xs">
									{data.stats.remoteContentFailed} failed · {data.stats
										.remoteContentPartial} partial
								</p>
							</Card.Content>
						</Card.Root>
					</a>
			...
			<div class="grid grid-cols-1 gap-4 lg:grid-cols-3">
				<div class=" lg:col-span-2">
					<Card.Root>
						<Card.Header>
							<Card.Title>{$t('app.dashboard_page.import_history')}</Card.Title>
						</Card.Header>
						<Card.Content class=" pl-4">
							{#if transformedHistory.length > 0}
								<IngestionHistoryChart data={transformedHistory} />
							{:else}
								<p>{$t('app.dashboard_page.no_import_history')}</p>
							{/if}
						</Card.Content>
					</Card.Root>
				</div>
				<div class=" lg:col-span-1">
					<Card.Root class="h-full">
						...
						<Card.Content class="h-full">
							{#if data.ingestionSources && data.ingestionSources.length > 0}
								<StorageBySourceChart
									data={data.ingestionSources}
									onSelect={openSourceMailbox}
								/>
							{:else}
								<p>{$t('app.dashboard_page.no_import_sources')}</p>
							{/if}
						</Card.Content>
					</Card.Root>
				</div>
				...
				<Card.Root>
					...
					<Card.Content>
						{#if data.indexedInsights && data.indexedInsights.topSenders.length > 0}
							<TopSendersChart
								data={data.indexedInsights.topSenders}
								onSelect={searchSender}
							/>
						{:else}
