<script lang="ts">
	...
	import { formatDate, formatRelativeToNow } from '$lib/stores/datetime.svelte';
					...
					{$t('app.license_page.revoked_message', {
						grace_period: data.licenseStatus.gracePeriodEnds
							? $t('app.license_page.revoked_grace_period', {
									date: formatDate(data.licenseStatus.gracePeriodEnds, {
										dateStyle: 'long'
									}),
								} as any)
			...
			<CardContent class="space-y-3 text-sm">
				...
				<div class="flex justify-between">
					...
					<span class="font-medium">
						{formatDate(data.licenseStatus.expiresAt, { dateStyle: 'long' })}
						({formatRelativeToNow(data.licenseStatus.expiresAt)})
					</span>
				</div>
				<div class="flex justify-between">
